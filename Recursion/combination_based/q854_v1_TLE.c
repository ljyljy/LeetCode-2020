/*
 难点：
 1) Queue<String> 队列 -> 双向链表<DLinkedNode> , struct NodeQ {String(char[]), idx, *nxt}
...
// typedef struct Deque { // NodeQ本身就是一个链表/链式队列！
//     NodeQ node; // Deque<NodeQ>
//     NodeQ* next;
//     NodeQ* prev;
// } Deque;
...
bool setFind(const char* key, HashSet* pEntry) {
    HASH_FIND_STR(set, key, pEntry);
    return pEntry == NULL;
}

bool setAdd(const char* key) {
    HashSet* pEntry = (HashSet*)malloc(sizeof(HashSet));
    // if (setFind(key) != NULL) return false;
...
static inline void swap0(char* s, int i, int j) {
    char tmp = s[i];
    s[i] = s[j];
    s[j] = tmp;
}
...
int kSimilarity(char* s1, char* s2) {
    ...
    setAdd(s1); // visitedSet.setAdd(s1);
    ...
    while (sizeQ > 0) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int j = curIdx + 1; j < n; j++) {
                ...
                if (curStr[j] == pattern) {
                    // swap(&curStr, curIdx, j);
                    // swap(&curStr, curIdx, j); // 此后，curStr实则为nxtStr
                    swap3(&curStr[curIdx], &curStr[j]);

                    // char* nxtStr = curStr; // 有误！不可放入Hash句柄！
                    HashSet* pEntry = NULL;
                    setFind(curStr, pEntry);
                    if (pEntry == NULL) {
                        setAdd(curStr);
